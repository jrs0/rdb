...
## Before running this script, you must set the working directory to
## the location of this script, and load_all or install the rdb package.
...
## Seed for randomness (predominantly in the models)
seed <- 47

## The proportion of the dataset used for model training
train_prop <- 0.75
...
## (levels no_bleed, bleed_occurred). The result is a dataset with age and
...
## Survival information across entire set
raw_dataset %>%
    dplyr::select(survival_time) %>%
    summary()
```

Put all the descriptive stuff here (plots, summaries, etc.) , before modifying the dataset for modelling.
...
## Check near-zero variance using caret (prints the
## remaining predictors)
predictor_columns <- dataset %>%
    dplyr::select(-nhs_number,-index_date,-index_id,-matches("after"))
near_zero_var_indices <- predictor_columns %>%
    nearZeroVar()
names(predictor_columns)[-near_zero_var_indices]

## These are the total counts of ACS/PCI events and patients
## excluding any rows containing an NA anywhere
num_index_events_exclude_na <- dataset %>%
    nrow()
num_patients_exclude_na <- dataset %>%
    distinct(nhs_number) %>%
    nrow()
...
bleeding_recipe <- recipe(bleeding_after ~ ., data = train) %>%
    ...
    step_integer(stemi_presentation, index_type) %>%
    step_nzv(all_predictors()) %>%
...
pred_bleeding <- list(names(models), models) %>%
        ...
        results %>%
            ...
            mutate(outcome_result =
                       recode_factor(bleeding_after,
                                     "bleeding_occurred" = "occurred",
                                     "no_bleed" = "none")) %>%
            rename(.pred_occurred = .pred_bleeding_occurred) %>%
        ...
        results %>% mutate(model_name = model_name) %>%
            ...
            mutate(outcome_result =
                       recode_factor(ischaemia_after,
                                     "ischaemia_occurred" = "occurred",
                                     "no_ischaemia" = "none")) %>%
            rename(.pred_occurred = .pred_ischaemia_occurred) %>%
            mutate(removals = if_else(model_id == "primary",
                                      list(removals = removals),
                                      list(removals = NULL)))
    }) %>%
...
## later
pred <- bind_rows(pred_bleeding, pred_ischaemia) %>%
    mutate(primary = case_when(model_id == "primary" ~ "primary",
                               TRUE ~ "bootstrap"),
           primary = factor(primary, levels = c("primary", "bootstrap"))) 
...
roc_curves <- grouped_pred %>%
    roc_curve(outcome_result, .pred_occurred)

## Compute the AUC
roc_auc <- grouped_pred %>%
    roc_auc(outcome_result, .pred_occurred) %>%
    rename(auc = .estimate) %>%
...
## Identifying variability in the fitted model
