...
class ColBinding {
public:
    ColBinding(SQLHSTMT hstmt, std::size_t col_index, std::size_t buffer_size)
	...
	SQLRETURN r = SQLBindCol(hstmt, col_index, SQL_C_TCHAR,
				 (SQLPOINTER)buffer_.get(),
				 buffer_size_, return_size_.get());
	try {
	    result_ok(hstmt, SQL_HANDLE_STMT, r);
...
#define COLUMN_NAME_BUF_LEN 100
	...
	ColBinding col_binding{hstmt_, index, column_length};
