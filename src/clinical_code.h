...
class ClinicalCodeMetagroup {
   public:
    ...
    bool contains(const ClinicalCodeGroup &group) const {
        return std::ranges::find(groups_, group) not_eq std::ranges::end(groups_);
    }
...
class ClinicalCode {
   public:
    ...
    /// by this id
    ClinicalCode(const CodeCacheEntry &data)
        : data_{data} {}
...
class ClinicalCodeParser {
   public:
    ClinicalCodeParser(const std::string &procedure_codes_file,
                       ...
                       std::shared_ptr<StringLookup> &lookup)
        : lookup_{lookup},
          procedure_parser_{YAML::LoadFile(procedure_codes_file), lookup},
          diagnosis_parser_{YAML::LoadFile(diagnosis_codes_file), lookup} {
    }
        ...
        try {
            switch (type) {
                case CodeType::Procedure: {
                    auto code_cache_entry{procedure_parser_.parse(raw_code)};
                    return ClinicalCode{code_cache_entry};
                }
                case CodeType::Diagnosis: {
                    auto code_cache_entry{diagnosis_parser_.parse(raw_code)};
                    return ClinicalCode{code_cache_entry};
                }
                default:
