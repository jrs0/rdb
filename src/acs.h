...
auto get_acs_and_pci_spells(const std::vector<Spell> &spells,
    ...
    auto is_acs_index_spell{[&](const Spell &spell) {
        ...
        return primary_acs(first_episode, acs_group) or primary_pci(first_episode, pci_group);
    }};
...
auto get_index_secondary_groups(const Spell &index_spell, CodeType type) {
...
auto get_spells_in_window(const std::vector<Spell> &all_spells,
    ...
    auto in_window{[&base_spell, offset_seconds](const Spell &other_spell) {
        ...
        if (offset_seconds > 0) {
            return (other_spell_start > base_start) and (other_spell_start < base_start + offset_seconds);
        } else {
            return (other_spell_start < base_start) and (other_spell_start > base_start + offset_seconds);
        }
    }};
